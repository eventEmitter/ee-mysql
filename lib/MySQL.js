	...
	module.exports = new Class( {
		...
		, query: function( query, parameters, callback, writable ){
			...
			if ( debug ) log.info( "got query ..." ), log.dir( query, parameters );


			var connection = this.__getConnection( writable );
			if ( connection ){
				if ( debug ) log.debug( "got a free connection for the query ..." );
				connection.query( query, parameters, callback );
			}
			else {
				if ( debug ) log.debug( "buffering query ..." );
				this.__buffer.push( { 
					type: 			"query"
					, query: 		query
		...
		, __getConnection: function( writable ){
			...
			if ( debug ) log.debug( "[" + i + "] existing connections ..." );

			while( i-- ) {
				if ( ! writable || this.__connections[ i ].isWritable() ) {
					connection = this.__connections.splice( i, 1 )[ 0 ];
					if ( debug ) log.debug( "returning existing connection ..." );
			...
			if ( debug ) log.debug( "no existing connection, creating one ..." );
			return null;
		}
		...
		, __setConnection: function( connection ){
			...
			if ( debug ) log.debug( "got a free connection ..." );

			// store connection
			this.__connections.push( connection );
			
			while( i-- ){
				if ( debug ) log.debug( " matching buffer [" + this.__buffer[ i ].writable + "] to connection [" + writable + "] ..." );
				if ( writable || !this.__buffer[ i ].writable ){
					if ( debug ) log.debug( " executing buffered query ..." );
		...
		, __createConnection: function( writable ){
			if ( debug ) log.debug( " creating a new connection ..." );
			...
			if ( debug ) log.debug( "failed to create new connection!" );
		}
					...
					, on: {
						...
						, connectionClose: function( connection ){
							this.__connections = this.__connections.filter( function( c ){
								if ( c === connection ){
									if ( debug ) log.debug( "deleting connection from available connections list ..." );
									return false;
								}
								return true;
							}.bind( this ) );
