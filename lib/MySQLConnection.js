	...
	module.exports = new Class( {
		...
		, query: function( query, parameters, callback ){
			...
			if ( debug ) {
				log.info( "<< --- QUERY ----------", "host: " + this.__config.host );
				console.log( query );
			...
			if ( debug ) log.debug( "starting query ..." );
			this.__connection.query( query, parameters, function( err, result ){
				if ( timing || debug ) log.debug( "query took [" + ( Date.now() - now ) + "] ms" );
				

				if( traceSlow && ( Date.now() - now ) > 100 ) {
					log.debug( "query took [" + ( Date.now() - now ) + "] ms" );
			...
			this.__idleTimeout = setTimeout( function(){
				this.emit( "requestRemove", function( removalAllowed ){
					if ( removalAllowed ){
						...
						if ( debug ) log.warn( "connection close [idle]" );
						this.__close();
					}
				}.bind( this ) );
		...
		, __connect: function(){
			...
			this.__connection.on( "error", function( err ){
				if ( debug ) log.error( "error on connection:" ), log.trace( err );
